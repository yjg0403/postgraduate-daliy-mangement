...
import com.yjg123.projectmanager.util.*;
...
public class StudentController {
    ...
    @Autowired
    private TeamService clazzService;
//    @Autowired
//    private SelectedCourseService selectedCourseService;
    ...
    public Object getStudentList(@RequestParam(value = "page", defaultValue = "1")Integer page,
                                 ...
                                 String teacherNo,
                                 String departName,
                                 String teamNo, String from, HttpSession session){
        Map<String,Object> paramMap = new HashMap();
        paramMap.put("pageno",page);
        paramMap.put("pagesize",rows);
        if(!StringUtils.isEmpty(studentName))  paramMap.put("username",studentName);
        if(!StringUtils.isEmpty(teacherNo))  paramMap.put("teacherNo",teacherNo);
        if (!StringUtils.isEmpty(departName)) paramMap.put("departName", departName);
        if (!StringUtils.isEmpty(teamNo)) paramMap.put("teamNo", teamNo);

        //判断是老师还是学生
        Student student = (Student) session.getAttribute(Const.STUDENT);
        if(!StringUtils.isEmpty(student)){
            //是学生权限，只能查询自己的信息
            paramMap.put("studentNo",student.getNo());
        }
...
//                }
//            }
            int count = studentService.deleteStudent(nos);
            if(count > 0){
...
//                fileImg.transferTo(newFile);
//
//                Student byId = studentService.findById(student.getNo());
//                File file = new File(fileDir.getAbsolutePath() + File.separator + byId.getPhoto());
//                if(file != null){
//                    file.delete();
//                }
